                ...
                {phase === 'photo' && (
                  <div className="max-w-md mx-auto">
                    ...
                    <label
                      ...
                      className="flex flex-col items-center justify-center gap-3 rounded-[20px] border border-dashed border-[#DED3D4] bg-[#FFFCFC] py-10 px-6 cursor-pointer hover:border-[var(--ink)] transition-colors text-center"
                    >
                      <UploadCloud size={22} className="text-[#AF9D9E]" />
                      <span className="text-sm font-medium text-[#564345]">
                        Drop your image here or <span className="text-blue-600 underline">browse</span>
                      </span>
                      <span className="text-xs text-[#AF9D9E]">Supports JPG &amp; PNG (4 images max)</span>
                      <input
                        id="photo-upload"
                        type="file"
                    ...
                    {/* TIP: this button is spec'd differently from every other
                        "Next" in the flow (mauve #AF9D9E, semibold 18px,
                        normal case — not the shared dark uppercase
                        PrimaryButton). Matched to the Figma frame exactly;
                        worth confirming with the client that this one-off is
                        intentional rather than a design slip. */}
                    <button
                      onClick={() => goTo('more')}
                      className="w-full mt-6 rounded-lg bg-[#AF9D9E] text-white text-lg font-semibold py-3 hover:bg-[#96827f] transition-colors cursor-pointer"
                    >
                      Next
                    </button>
                  </div>
