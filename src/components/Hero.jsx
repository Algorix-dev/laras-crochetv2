...
const isSampleModel = (model) =>
  String(model.id).startsWith("fallback-");
...
/*
  MOBILE CENTER MODEL SIZE

  The mobile stage already uses the available dynamic viewport
  height, so the model is allowed to fill that stage naturally.

  Keep this at 1 so the model does not push its head unnecessarily
  far into the navbar area.
*/
const SELECTED_SCALE_PHONE = 1.0;
...
/*
  Space reserved below the model stage.

  This is what gives the podium + price row room to exist without
  forcing the page to scroll just to reveal the price.
*/
const MOBILE_BOTTOM_RESERVE_PX = 110;

/*
  Space between the bottom of the model stage and the price row.
*/
const MOBILE_PRICE_GAP_PX = 24;
...
/*
  Side models: 3D glassy depth-of-field blur & opacity.
*/
const SIDE_MODEL_OPACITY = 0.50;
const SIDE_MODEL_BLUR_PX = 4.5;

/*
  How close the models sit.
*/
...
/*
  Desktop:
    clamp(25rem, 34vw, 42rem)

  Mobile:
    The model stage fills the available dynamic viewport height.
  ...
  The calculation is:
    ...
    - navbar
    - name gap
    - bottom area for podium + price

  This means the model gets the remaining space instead of
  relying on a fixed 27rem height.
*/
...
function wrapOffset(value, count) {
  ...
  return (
    ((((value - lowest) % count) + count) % count) +
    lowest
  );
}
